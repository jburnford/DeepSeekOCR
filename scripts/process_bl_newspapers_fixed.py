#!/usr/bin/env python3
"""
Process British Library Newspaper Images with DeepSeek-OCR (FIXED VERSION)
Evaluate against ground truth data from Gale's collection.

FIXES:
- Captures stdout from model.infer() which prints structured output
- Parses structured output to extract plain text
- Properly saves OCR text to JSON files
"""
...
def parse_deepseek_output(raw_output: str) -> str:
    ...
    DeepSeek outputs text with XML-like tags:
    <|ref|>text<|/ref|><|det|>[[x1,y1,x2,y2]]<|/det|>
    Actual text content here...

    This function extracts just the text portions.
    """
    text_parts = []

    # Pattern for text after </det|> tags
    # Text continues until next <|ref|| or end of string
    text_pattern = r'<\|/det\|>\n(.+?)(?=\n<\|ref\||$)'

    for text_match in re.finditer(text_pattern, raw_output, re.DOTALL):
        text = text_match.group(1).strip()
        # Remove markdown header markers if present
        text = re.sub(r'^## ', '', text)
        # Skip very short strings (likely artifacts)
        if len(text) > 10:
...
class BLNewspaperProcessor:
    ...
    def process_image(self, image_path: Path) -> Dict[str, Any]:
        ...
        # Capture stdout since model prints output there
        ...
        # Parse the captured output to extract plain text
        extracted_text = parse_deepseek_output(stdout_content)

        # If no text extracted, fall back to result (might be None)
        ...
        return {
            ...
            "raw_output_length": len(stdout_content)  # For debugging
        }
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"British Library Newspaper OCR - DeepSeek-OCR (FIXED)")
    print(f"{'='*70}")
